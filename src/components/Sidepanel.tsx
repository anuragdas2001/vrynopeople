...
const Sidepanel = ({ isCollapsed }: { isCollapsed: boolean }) => {
  const router = useRouter();
  const [activeTab, setActiveTab] = useState<string>("/");
  console.log("activeTab", activeTab);
  useEffect(() => {
    const path = `/${router.pathname.split("/")[1]}`; // Ensure the path has a leading '/'
    setActiveTab(path || "/"); // Fallback to '/' if path is empty
  }, [router.pathname]);
  ...
  // Utility function to generate button classes consistently
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-2 mb-4">
        <img
          className={`h-10 w-10 ${isCollapsed ? "ml-3" : ""}`}
        ...
        />
        {!isCollapsed && (
          <h1 className="text-xl font-bold text-blue-500">VrynoWorkForce</h1>
        )}
      </div>
